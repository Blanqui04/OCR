...
class OCRPipeline:
    def __init__(self, base_dir=None):
        ...
        # Inicialitzar detector d'elements tècnics
        try:
            self.technical_detector = TechnicalElementDetector()
            self.use_technical_detection = True
        except Exception as e:
            print(f"⚠️ Detector d'elements tècnics no disponible: {e}")
            self.technical_detector = None
            self.use_technical_detection = False
    ...
    def process_pdf(self, pdf_path, save_files=True):
        ...
        # 5. Detecció d'elements tècnics amb YOLOv8
        technical_elements = {}
        if self.use_technical_detection and self.technical_detector:
            try:
                technical_elements = self.technical_detector.detect_elements(
                    image_path, 
                    save_annotated=True
                )
                results['technical_elements'] = technical_elements
                
                if save_files:
                    with open(os.path.join(self.base_dir, "data/technical_elements/yolo_detections.json"), "w", encoding="utf-8") as f:
                        json.dump(technical_elements, f, ensure_ascii=False, indent=2)
            except Exception as e:
                print(f"⚠️ Error en detecció d'elements tècnics: {e}")
                technical_elements = {"error": str(e)}
        
        # 6. Extracció tècnica (OCR tradicional)
    ...
    def get_stats(self, results):
        ...
        stats = {
            ...
            'yolo_elements': results.get('technical_elements', {}).get('total_elements', 0),
            'yolo_summary': results.get('technical_elements', {}).get('summary', {})
        }
        return stats
...
def main():
    ...
    print(f"  - Elements YOLO: {stats['yolo_elements']}")
    print(f"  - YOLO per tipus: {stats['yolo_summary']}")
